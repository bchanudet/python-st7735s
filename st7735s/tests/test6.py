...
# Images go beyond the LCD size, so they get cropped
for i in range(10):
    screen.draw_at( G.L_IMG[G.L_IDX], x=0, y=0 )
    G.L_IDX = (G.L_IDX + 1) % G.L_SIZE

    screen.draw_at( G.L_IMG[G.L_IDX], x=70, y=5 )
    G.L_IDX = (G.L_IDX + 1) % G.L_SIZE

    screen.draw_at( G.L_IMG[G.L_IDX], x=74, y=74 )
    G.L_IDX = (G.L_IDX + 1) % G.L_SIZE

    screen.draw_at( G.L_IMG[G.L_IDX], x=20, y=84 )
    G.L_IDX = (G.L_IDX + 1) % G.L_SIZE
